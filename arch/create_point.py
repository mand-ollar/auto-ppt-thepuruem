...
type_writer(
    "Which stands for,, uh,, Christian Coding Club.(Hope you make new facinating name for this.)"
)
time.sleep(alpha * 0.5)

type_writer(
    "It makes ppt file from template in any way that you want.(Sequence, conponents, etc.)"
)
time.sleep(alpha * 0.5)
...
type_writer(
    "You must make sure that the txt file 'Layout Sequence List' be in this directory with the same name as template file."
)
...
# If there's txt files in the directory
    ...
    type_writer(
        "Choose the file if you already have on in this directory. If not, just choose 0.\n"
    )
...
if selection == 0:
    ...
    time.sleep(alpha * 0.5)
    type_writer(
        "Please make sure that the list is based on the exact name of these layouts.\n"
    )
    time.sleep(alpha * 1)
    ...
    time.sleep(alpha * 1)
    type_writer(
        "1. Praise and Worship section: You have to write 'praisenworship' to the praise and worship section."
    )
    time.sleep(alpha * 0.5)
    type_writer(
        "2. Sermon section: You have to write 'allofsermon' to the section from sermon title to the end."
    )
    time.sleep(alpha * 0.5)
    type_writer(
        "3. After sermon praise section: You have to write 'aftersermon' to the after sermon praise section."
    )
    time.sleep(alpha * 0.5)
    type_writer(
        "4. Normal praise section: You have to write 'normalpraise' to the normal praise section."
    )
    ...
    with open(working_dir / f"dict/{json_name[0]}", "r") as session_json:
