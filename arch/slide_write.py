...
class slideWrite:
        ...
        if layout_name == "allofsermon":
            ...
            for k in range(len(bibles)):
                for i in range(len(secs[k])):
                    ...
                    secnum = secs[k][i]
                    phrase = self.gp.get_phrase(
                        version=self.variable_dict["version"],
                        book=books[k],
                        chap=chaps[k],
                        sec=secnum,
                    )
                    ...
                    for j in range(ph_cnt):
                        ph = self._find_placeholder_by_name(
                            f"Text Placeholder {j+1}", phs
                        )
                        ph.text = self.variable_dict[
                            self.placeholer_dict[layout_name][j]
                        ]
    ...
    def _prepare_lyrics_(
        ...
        while loop == True:
            ...
            self.get_lyrics.get_lyrics(
                song_title=title, artist_name="thisisnotanartistname"
            )
            self.current_dir = Path(__file__).parent
            os.system(f"nano {self.current_dir}/lyrics/txt/{title_for_bash}.txt")
            loop = self.get_lyrics.get_lyrics(
                song_title=title, artist_name="thisisnotanartistname", print_it=False
            )
            ...
            if loop:
                ...
                while key_input == "thisisnotaninput":
                    ...
                    if key_input == "":
                        ...
                        Path(f"{self.current_dir}/lyrics/txt/{title}.txt").unlink()
                        os.system("clear")
    ...
    def write_worship(
        ...
        if type == "remain":
            ...
            if Path(self.current_dir / f"lyrics/json/{title}.json").exists():
                with open(
                    self.current_dir / f"lyrics/json/{title}.json", "r"
                ) as lyrics_json:
